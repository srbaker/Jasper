...
describe('mergeCatalog', () => {
  ...
  it('adds a user entry not in the seed', () => {
    const mine: RowanCatalogEntry = {
      name: 'MyLib', description: '', projectUrl: '', baseline: 'MyLib',
      repository: 'github://me/MyLib:master/repository', loads: ['default'],
    };
  ...
  it('lets a user entry override a seed entry of the same name', () => {
    const override: RowanCatalogEntry = {
      name: 'Seaside', description: 'pinned', projectUrl: '', baseline: 'Seaside3',
      repository: 'github://SeasideSt/Seaside:v3.5.9/repository', loads: ['Core'],
    };

    const merged = mergeCatalog([override]);

    expect(merged.length).toBe(CATALOG_SEED.length);
    expect(merged.find(e => e.name === 'Seaside')?.repository).toBe('github://SeasideSt/Seaside:v3.5.9/repository');
  });
});
