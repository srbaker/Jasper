/**
 * Cucumber JSON → normalized manual model + extracted screenshot bodies. Generic
 * and product-agnostic (see `cucumberJson.ts`).
 *
 * `buildManual` is pure (no fs, no clock): raw report in, model + asset bodies
 * out — so it is trivially testable and its output diffs cleanly. The CLI writes
 * the assets to disk and the html renderer turns the model into the page.
 */
